= Поробуйте, наконец, Celesta в своём проекте!
Иван Пономарёв
...
image::celesta_duke.png[{image-50-width}]

* Ответ на сложность разработки и тестирования систем с базами данных
* Миграции + Type-Safe code-generated ORM + testing
...
* image:inlove.png[] Вы только посмотрите на документацию: https://courseorchestra.github.io/celesta/en/index.html#CelestaSQL[courseorchestra.github.io/celesta]
* image:thumbs-up.png[]  IDEA Plugin
...
* image:shrug.png[] Мало звёзд на гитхабе -- подкиньте, пожалуйста image:star.svg[width=32]!
...
2. Всегда можно просверлить дырку в абстракции
...
[%step]
. На практике, вы нескоро встретите случай, когда idempotent DDL миграция не справится

. Да, иногда миграцию нужно производить вручную. Если число окружений, на которых _нужно_ производить миграцию, не больше двух, у вас _нет проблемы_

. Кейс: множество окружений. Celesta + Flyway. Можно отключить мигратор Celesta и использовать автоматический вместо него

. Автоматические миграторы, кстати, тоже иногда не срабатывают на проде
...
for (FooCursor c: foo) {
  if (!Objects.equals(bar.getId(), c.getBarId()))
  /* минимальное число обращений к базе */
     bar.get(c.getBarId());
}
----
